...
PoseGraph::PoseGraph()
{
    ...
    std::ofstream outFile("output/pose_graph_optimization_time.txt");
    outFile.close();
}
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            float pose_optimization_time = tmp_t.toc(); 

            printf("pose optimization time: %f \n", tmp_t.toc()); 

            std::ofstream outFile("output/pose_graph_optimization_time.txt", std::ios::app);
            outFile << pose_optimization_time << std::endl;
            outFile.close();
            /*
            for (int j = 0 ; j < i; j++)
            {
